...
#---- begin ---Evan Weiler-----
	...
	def getShortestPath(self, node1, node2):
		...
		if node1 < self._numVerts and node2 < self._numVerts:
			unchecked = list(range(self._lastNode))
			distances = {}
			for node in unchecked:
				distances[node] = (sys.maxint, None)	
			distances[node1] = (0, None)
			current = node1	
			while current != node2:
				distanceToCurrent, prev = distances[current]
				neighbors = self.getNeighbors(current)
				for neighbor, distance in neighbors:
					if neighbor in unchecked:
						thisPathDist = distanceToCurrent + distance
						currentNeighborLowDist, prev = distances[neighbor]
						if thisPathDist < currentNeighborLowDist:
							distances[neighbor] = (thisPathDist, current)
				unchecked.remove(current)
				minDist = sys.maxint
				for node in unchecked:
					distance, prev = distances[node]
					if distance <= minDist:
						minDist = distance
						current = node			
			shortestPath = collections.deque()
			while True:
				if current == None:
					break
				shortestPath.appendleft(current)
				distance, current = distances[current]				
			return tuple(shortestPath)
		elif node1 >= self._numVerts:
			raise NodeIndexOutOfRangeException(0, self._numVerts, node1)
		else:
			raise NodeIndexOutOfRangeException(0, self._numVerts, node2)
