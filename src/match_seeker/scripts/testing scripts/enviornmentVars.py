...
# Adds the hostname to the .bashrc file
def set_hostname(socketname):
...
# Adds the ros_master_uri to the .bashrc file
def set_ros_master_uri(uri):
    bashrc_path = os.path.expanduser("~/.bashrc")
    with open(bashrc_path, "a") as bashrc:
...
set_ros_master_uri(new_uri)
set_hostname(IPSocketName)
